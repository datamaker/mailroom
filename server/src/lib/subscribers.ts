...
export async function upsertSubscriber(listId: string, input: UpsertInput): Promise<UpsertOutcome> {
  ...
  if (existing) {
    ...
    if (input.by === 'SUBSCRIBER' && (status === 'unsubscribed' || status === 'deleted')) {
    ...
    await query(
      `update subscribers
          set fields = $2::jsonb,
              ...
              status = $4,
              subscribed_at = case when status <> 'subscribed' and $4 = 'subscribed' then now() else subscribed_at end,
              unsubscribed_at = case when $4 = 'subscribed' then null else unsubscribed_at end,
              updated_at = now()
        where id = $1`,
      [existing.id, JSON.stringify(merged), input.adAgreed ?? null, status]
    );
    if (input.groupIds?.length) await addToGroups(existing.id, input.groupIds);
    return { id: existing.id, email, result: 'updated' };
  }

  const created = await one<{ id: string }>(
    `insert into subscribers (list_id, email, status, ad_agreed, fields, source)
     values ($1, $2, $3, coalesce($4,false), $5::jsonb, $6)
     returning id`,
    [listId, email, input.pending ? 'pending' : 'subscribed', input.adAgreed ?? null, JSON.stringify(incoming), input.source ?? 'api']
  );
  if (input.groupIds?.length) await addToGroups(created!.id, input.groupIds);
